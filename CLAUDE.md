...
_Generated by `tools/gen-code-map.py` from `src/` (170 source files). Do not edit by hand — re-run the script._
...
| `src/doc/js/0730-doc-gen.js` | **App.docGen — DOC-GEN: the document, built from a host's declarations** | Turn what a HOST declares — its sections, its subject, its filter — into the blocks a document is made of, the columns each block prints and the mark… |
...
- **`App.docGen`** — `reportBlocks`, `sectionColumns`, `sectionContent`, `filterCounts`, `findTags`, `applyTags`, `docFilename`, `TAG_RE`, `sectionsOf`, `providerOf`
...
### Self-test blocks (16 blocks, 196 suites)
...
**(unlabelled suites at line 34339)** — `src/doc/js/0900-doc-suites/020-blocks.js`

- _the block helpers (BLK-1)_
  - `line 11` BLK-1 ordering and wording are the module’s, not the host’s

**(unlabelled suites at line 34402)** — `src/doc/js/0900-doc-suites/030-host-object.js`

- _the host contract (HOSTOBJ-1)_
  - `line 10` HOSTOBJ-1 the module is driven by an explicit host object

**(unlabelled suites at line 34479)** — `src/app/js/0600-v2-2-self-test-suites-report-design-markdown/` (17 files)
...
**(unlabelled suites at line 39643)** — `src/app/js/0610-suites-paths-and-the-driver-contract-tf-2/` (3 files)
