...
_Generated by `tools/gen-code-map.py` from `ch-config-tool.html` (38,902 lines). Do not edit by hand — re-run the script._
...
| `17775–18584` | **App.ui.views.help** | In-app manual (spec §11, Phase 8; rewritten review-15). One section per area of the app, reached from a section strip so no single page is a wall of… |
| `18588–18991` | **App.ui.folder — TF.6/TF.8/TF.9/TF.11: the folder, as the app sees it** | The single seam between the storage layer and the rest of the UI. Owns the folder store and the writer, the connect/reconnect/recovery/divergence scr… |
| `18993–20805` | **App.ui.app** | The UI shell + controller (spec §11.1). Builds the top bar, the data-driven tab nav (one tab per active-platform dataset + Devices/ Onboard/Generate)… |
| `27933–29057` | **App.docStore — DS-1..DS-6: the Report Design mutators** | Every write the Report Design workspace makes to the project — heading levels, custom sections and their parts, formatting profiles, section template… |
| `29059–29316` | **App.docTemplates — TPL-1..TPL-4: templates in and out, on their own** | Export and import section templates, formatting profiles and whole report templates as standalone files, independently of the project. Owns the merge… |
| `29318–29920` | **App.ui.mdPreview — PRV-1: markdown -> HTML for the live preview** | Render the EXACT markdown the Generate button downloads into browsable HTML, plus the outline needed to navigate it. Not a general markdown implement… |
| `29922–30226` | **App.ui.richText — RTX-1: the box shows what it holds** | Turn the {{...}} token markup App.md.rich reads into HTML a contenteditable box can be edited in, and turn the edited box back into tokens. Bold read… |
| `30228–32943` | **App.ui.views.reportDesign — RD-1..RD-7: the Report Design workspace** | The full-screen workspace that decides what the document contains, in what order, at what heading level, with what hand-authored sections, under whic… |
| `38844–38899` | **App.bootstrap — LAST. Registers platforms and mounts the UI.** | Wire everything together at startup (spec §14). During Phase 0 it only renders self-tests on #selftest and a placeholder otherwise. |
...
**PHASE-3 SELF-TEST SUITES (ui.model + ui.tables render)** — lines `20808–20916`

- `20846` ui.model
- `20881` ui.tables render

**PHASE-4 SELF-TEST SUITES (diff · validation · store onboarding)** — lines `20919–21061`

- `20940` diff.triage
- `20949` validation.validateOnboarding
- `20961` store.onboardDevice (fresh + inheritance)
- `21005` store re-onboard versioning (§8.7)
- `21043` store.applicableItems / undecidedCount

**PHASE-5 SELF-TEST SUITES (completeness · store decisions · editors · gating)** — lines `21064–21212`

- `21094` completeness
- `21120` store.setDecision / setItemFields
- `21162` ui.tables decision editors (data-driven)
- `21195` ui.views.generate gating

**PHASE-6 SELF-TEST SUITES (devices view)** — lines `21215–21337`

- `21241` ui.views.devices list
- `21262` ui.views.devices detail (DOD-9)
- `21295` review-5 #1 device panels collapsible + per-control view

**PHASE-7 SELF-TEST SUITES (report · generate · determinism)** — lines `21340–21828`

- `21367` report markdown builders (v2.2)
- `21398` generate.buildImplementation
- `21437` generate.buildVerification & buildReport
- `21554` report-gen: descriptions + page setup + LaTeX safety (v2.2)
- `21607` T11 (v1.3) generation customisation
- `21738` review-9 apply-to toggle + AEST header + how-to-run
- `21774` generate determinism (DOD-7)
- `21795` review-10 (onboarded descriptions + .ps1 warning + control width)

**REVIEW-3 SELF-TEST SUITES** — lines `21831–21946`

- `21848` review-3 #1 generate scripts as .txt
- `21863` review-3 #3 / review-4 #1 policyList (prefixed internal key, stripped for display)
- `21912` review-3 #5a/#5b control types + import
- `21933` review-3 #6 device-detail search

**PHASE-8 SELF-TEST SUITES (DOD-11 portability + DOD checklist)** — lines `21949–22059`

- `22001` DOD-11 portability (mock platform, zero core edits)
- `22040` DOD checklist (programmatic items)

**PHASE-9 SELF-TEST SUITES (v1.1: dark mode · set-from-files · controls)** — lines `22062–27928`

- _v1.1 core (T9.x) — dark mode · set-from-files · control CRUD_
  - `22068` T9.1 dark mode (UI-only, determinism-safe)
  - `22093` T9.2 adapter parseAssignment
  - `22114` T9.3 store.applyDeviceAssignment (exact-set + atomic)
  - `22157` T9.6 store control CRUD
- _v1.2 overrides & device groups (T10.x)_
  - `22184` T10.3 store override + group mutators
  - `22251` T10.4 effective completeness, generation & manifest
  - `22296` T10.6 device-config override UI (divergence + editing)
  - `22348` T10.5 device groups UI (sections + deviations modal)
  - `22405` T10.7/T10.8 report deviations + determinism (overrides/groups)
- _review rounds 2-4, 7-8 — UI refinements + control refs in tables/report_
  - `22455` review-2: AEST banner time + resizable/wrapping value columns
  - `22477` review-7 collapse-all + Apply Control Mode
  - `22532` review-8 name-only picker + Apply-to action + resizable panels
  - `22595` T9.8/T9.9 control refs in tables & report
  - `22631` review-4 #2/#3/#4 UI: incomplete default + control layout/description
- _review rounds 12-13 — satisfaction · delete mode · relevance · undo/redo_
  - `22714` review-12 #1 per-device control satisfaction state
  - `22789` review-12 #2 Delete Mode + Undo
  - `22871` review-12 #3 Security Relevance column
  - `22923` review-12 #4 rationale + relevance columns in the decisions import
  - `22987` review-13 #1 delete selection tint
  - `22997` review-13 #2/#3 apply runs undo as one action, plus redo
  - `23066` UNDO-1 universal undo
  - `23174` review-13 #4 Control Manager per-device checkbox columns
- _review rounds 14-17 — relevance options · column widths · manual · editors_
  - `23252` review-14 REPORT + IRRELEVANT relevance options
  - `23360` review-15 starting column widths
  - `23393` review-15 Help manual
  - `23449` review-16 #1 blanking a text value
  - `23512` review-17 #1 tab order
  - `23537` review-17 #2 clear button sits beside the value box
  - `23550` review-17 #3 the status badge flips the item
  - `23623` review-17 #4 imsSettings is optional on upload
- _v2.1 value formats (VF-1…VF-8)_
  - `23735` VF value formats — inference, editors, enforcement
- _bulk apply & full-cell hit targets (BULK-1/2)_
  - `23991` BULK-1 apply the selected control to everything SHOWN
  - `24089` BULK-2 the tick columns are full-cell hit targets
- _hold-for-review · column filters · justification (HELD-1 · FIL-1 · JUS-1/2)_
  - `24129` HELD-1 flag for review without losing the value
  - `24242` FIL-1 per-column value filters
  - `24424` JUS-1/JUS-2 control satisfaction + justification
- _layout stability on tick (STAB-1 · STAB-3)_
  - `24574` STAB-1 a tick never changes a row height
  - `24639` STAB-3 a tick in the Control Manager does not move the page
- _control tags & column-wide device assign (TAG-1/2/3)_
  - `24667` TAG-3 the device column heading assigns the whole shown column
  - `24732` TAG-1/TAG-2 custom tags on controls
- _the sticky tools rail (SP-4 · SP)_
  - `24860` SP-4 the rail is actually sticky (CSS invariants)
  - `24975` SP sticky tools rail
- _v2.0 — retirement of the Settings dataset_
  - `25064` v2.0 Settings retirement (legacy projects still load)
- _columns & device read-across (CMD-1 · COL-1)_
  - `25198` CMD-1 the Applies-to device names read across, not down
  - `25252` COL-1 every column is hidable except the key column
- _divergence from guidelines (DIV-1/DIV-2)_
  - `25367` DIV-1/DIV-2 diverges from guidelines + the narrative behind it
- _custom security actions (CUS-1/CUS-2)_
  - `25538` CUS-1/CUS-2 Custom Security Actions
- _security relevance renamed (REL-1)_
  - `25816` REL-1 Security Relevance 
- _general platform notes (NOTE-1)_
  - `25884` NOTE-1 General Platform Notes
- _procedures & the Procedure report (PRO-1 · PRO-2)_
  - `25983` PRO-1/PRO-2 Procedures and the Procedure report
- _Control Manager column filters (CMF-1)_
  - `26148` CMF-1 Control Manager column filters
- _firewall rules & USB host interfaces (FW-1 · USB-1)_
  - `26254` FW-1/USB-1 firewall rules and USB host interfaces
- _column defaults · control coverage · order stability (COL-2 · RPT-1 · PRO-3)_
  - `26419` COL-2/RPT-1/PRO-3 columns, control coverage and the order editor
- _cell editing & manual device assignment (EDIT-1 · DEV-1)_
  - `26570` EDIT-1 a cell opens the row for editing
  - `26620` DEV-1 assigning register items to a device by hand
- _one captured-value map for the whole fleet (D-015)_
  - `26925` D-015 an assigned item keeps its decision on the new device
- _one stubborn package must not end the run (D-016)_
  - `27011` D-016 a package that refuses to uninstall must not kill the run
- _uninstall falls back to disable · reverse switches (PARTIAL-1 · REV-1)_
  - `27062` PARTIAL-1 a refused uninstall falls back to disable, and says so
  - `27107` REV-1 the reverse switches turn the script into a restore
- _a verification script carries no apply machinery (VER-5)_
  - `27168` VER-5 the verification script contains nothing that could change a device
- _the "(none)" filters survive the DOM (FIL-3)_
  - `27255` FIL-3 a 
- _report scope, section order & the options workspace (REL-7 · RPT-2/3/4)_
  - `27380` REL-7 
  - `27416` RPT-2 the report carries only the relevance categories you ask for
  - `27465` RPT-3 the report sections can be re-ordered, and it sticks
  - `27534` RPT-4 / RD-1 the Report Design workspace
- _bulk relevance & bulk decision (BULK-4)_
  - `27630` BULK-4 applying a relevance or a decision to many rows
- _a sort you can turn off (SORT-1)_
  - `27840` SORT-1 a column heading cycles ascending, descending, off

**v2.2 SELF-TEST SUITES (Report Design: markdown · outline · templates)** — lines `32946–38107`

- _LaTeX-safe markdown (MD-1)_
  - `32987` MD-1 nothing hostile to LaTeX leaves the writer unescaped
- _heading levels, numbering and cross-references (DOC-1..DOC-4)_
  - `33050` DOC-1/DOC-2 heading levels resolve and number themselves
  - `33105` DOC-3 a cross-reference survives renaming and reordering
  - `33159` DOC-4 a hand-authored section renders its parts in order
- _formatting profiles (FMT-1..FMT-4)_
  - `33208` FMT formatting profiles reach pandoc as YAML + a LaTeX preamble
- _the designer's project writes (DS-1..DS-6)_
  - `33274` DS the Report Design mutators write deterministic, canonical state
- _templates in and out (TPL-1..TPL-4)_
  - `33366` TPL importing templates adds; it never deletes
- _the document the designer produces (RD-1..RD-7 · PRV-1)_
  - `33468` RD a designed document generates as one .md
- _metadata rows, guideline deviations, centring, section preview_
  - `33551` META-1 the provenance block is a chooseable list
  - `33600` GUIDE-1 Deviations from Security Guidelines
  - `33654` RD-8 centring and the per-section preview
  - `33704` PRV-1 the preview renders the document that will be generated
  - `33761` RD-2 the section editor exposes every part control
- _table column widths (TW-1)_
  - `33828` TW-1 a dragged column width reaches the PDF
- _automatic widths and the form a table takes (AUTO-1)_
  - `34038` AUTO-1 a table that will not fit is laid out, not left to collapse
- _header + first-column styling (TBS-1)_
  - `34110` TBS-1 a styled header row and first column
- _generated-section columns and widths (CCOL-1 · TW-2 · RD-9)_
  - `34337` CCOL-1/TW-2 a generated section chooses its columns and their widths
  - `34456` TW-1 typing a width, and being told when it does not add up
  - `34545` RD-9 a hand-authored section previews like a generated one
- _the width model measures ems (AUTO-2)_
  - `34591` AUTO-2 columns are measured in ems, not in characters
- _quality of life (FNT-1 · OPT-1 · pane stickiness · tab)_
  - `34657` FNT-1 three font sizes: text, table body, table header
  - `34778` OPT-1 columns and groups live on the section row
  - `34822` the workspace stops moving under you
- _long runs are marked so they can break (BRK-1)_
  - `34853` BRK-1 a run with no space in it is marked so it can wrap
- _what the preview shows (PRV-3)_
  - `34928` PRV-3 the preview reads the document back the way the page will
- _a table narrower than the page (TW-3)_
  - `34989` TW-3 widths that do not fill the page make a narrower table
- _an unnumbered title level (TTL-1)_
  - `35044` TTL-1 a title takes no number and gives none away
- _the profile reaches the preview (PRV-2 · CAP-2 · NAM-2)_
  - `35115` PRV-2 the preview shows the formatting, not just the words
  - `35187` NAM-2/CCOL-2 a generated section names its heading; a control names its type
- _automatic captions (CAP-1)_
  - `35280` CAP-1 every table is captioned, and the numbers agree
- _section names and introductions (NAM-1 · SEC-1)_
  - `35367` NAM-1/SEC-1 a section names itself, and introduces itself
- _a numbered introduction (SEC-2) · control description (CCOL-3)_
  - `35480` SEC-2 an introduction can take a number of its own
  - `35583` CCOL-3 the control coverage table can carry the description
- _values a person reads (HUM-1)_
  - `35626` HUM-1 a captured value is printed as a reading, not as JSON
- _satisfied with exception (EXC-1)_
  - `35699` EXC-1 a control can be satisfied with an exception
- _the body font (FNT-2)_
  - `35789` FNT-2 the document chooses a font, and the preview shows it
- _the document the designer controls (CODE-1 · TOC-1/2 · TTL-2 · CAP-3)_
  - `35863` CODE-1 a code span is shaded, and a long one still wraps
  - `35902` TOC-1/TOC-2 the contents list is a section like any other
  - `35986` TTL-2 the automatic title block is a choice, and it is off
  - `36026` CAP-3 the caption sits under its table, and carries its number
- _the box shows what it holds (RTX-1 · RTX-2)_
  - `36101` RTX-1 a text box renders what it holds
  - `36244` RTX-2 a table cell takes the same formatting a paragraph does
- _what the report is made of, saved with it (OPT-2 · COL-3)_
  - `36289` OPT-2 the report\
  - `36389` COL-3 an optional column carries its own default
- _a table without a caption (CAP-4)_
  - `36430` CAP-4 a table can be left uncaptioned
  - `36510` SEC-3 the report no longer describes its own composition
  - `36543` NAM-3 heading, then name, whoever wrote the section
  - `36570` FNT-3 every size in the document, in one table
  - `36673` TTL-3 a title is styled as a title, not as an H1
- _references that reach the PDF (REF-1 · REF-2)_
  - `36745` REF-1 a cross-reference reaches the document as a link
  - `36836` REF-2 every mention of a control links to its coverage row
- _type, centring, table wording and placement (FNT-4 · CTR-1 · TBL-1 · SEC-4)_
  - `36924` FNT-4 every kind of text has a size, a weight and a slope
  - `36965` CTR-1 centring a section centres its heading too
  - `37046` TBL-1 a generated table names itself, and its groups take no heading
  - `37188` SEC-4 a section can start a page, or stay out of the contents
- _header & footer, generation, the paged preview (HDR-1 · GEN-TAB · PRV-4)_
  - `37288` HDR-1 the header and footer are configured on their own
  - `37442` GEN-TAB the document is named, and its placeholders filled in
- _what the second round of use found (D-036..D-039)_
  - `37499` D-036 a hand-set width still holds what cannot be broken
  - `37573` D-044 centring one section does not centre the document
  - `37620` BR-1 a line break renders wherever it is written (D-037)
  - `37658` PRV-4 the preview can be laid out as pages
- _a break in a cell, and a first column with a size (D-061 · FNT-6)_
  - `37695` D-061 a line break in a table cell reaches the preview
  - `37753` FNT-6 the table\
- _whitespace as a measurement (SPC-1)_
  - `37830` SPC-1 empty space is asked for in millimetres

**(unlabelled suites at line 38109)** — lines `38109–38842`

- _paths and the driver contract (TF.2)_
  - `38183` TF.2 storage path helpers
  - `38202` TF.2 driverMemory honours the driver contract
- _the snapshot policy (TF.4 · requirements 7.1)_
  - `38246` TF.4 snapshots are due on a 5-minute floor and pruned to 40
- _the canonical write (TF.4 · requirements 6.2, 7.1)_
  - `38306` TF.4 the canonical write
- _the concurrent-write guard (TF.4 · requirements 10)_
  - `38361` TF.4 the guard refuses to clobber
- _quarantine and outputs (TF.4 · requirements 8, 9)_
  - `38436` TF.4 quarantine preserves the bytes a human could repair
  - `38468` TF.10 generated artifacts land unpacked under Outputs/
- _the debounced writer (TF.5 · requirements 6.2)_
  - `38502` TF.5 the writer coalesces, caps, and never spins
- _the app wired to a folder (TF.6 · TF.8 · TF.10)_
  - `38623` TF.6 the app boots into the right folder state
- _the writer over the real folder store (TF.4 + TF.5)_
  - `38784` TF.5 the writer driving the folder store end to end
