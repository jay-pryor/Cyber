...
- **The document module is host-agnostic.** Nothing under `src/doc/` may reference a name defined
  under `src/app/` — `tools/build.py` fails the build if it does, and `BOUNDARY_DEBT` (the allowlist
  that held the line during the extraction) is empty and asserted empty. The host hands the module
  one object through `App.docHost.set(...)`; the module never reaches back. A section is a
  PROVIDER the host declares, not a `kind` the module knows about. See
  `docs/superpowers/specs/2026-09-08-document-designer-module-design.md`, the worked example in
  `README.md` ("Using the document module in another app"), and the CONF-1 suite that proves a
  foreign host generates a document with no core edits.

Verify with the embedded suite: open `ch-config-tool.html#selftest` (must be all green), or run it
headlessly with `node tools/run-selftests.js`. The document module has its own runner —
`node tools/run-module-selftests.js` loads `doc-designer.js` with no `src/app/` code present, which
is what proves its suites are portable rather than merely passing inside CH.
...
_Generated by `tools/gen-code-map.py` from `src/` (176 source files). Do not edit by hand — re-run the script._
...
| `src/doc/js/0700-doc-host.js` | **App.docHost — the contract between the module and its host application** | Hold the one object the host supplies, and say plainly when it is malformed. Everything the module needs from the outside world arrives here; nothing… |
...
| `src/doc/js/0730-doc-gen/` (2 files) | **App.docGen — DOC-GEN: the document, built from a host's declarations** | Turn what a HOST declares — its sections, its subject, its filter — into the blocks a document is made of, the columns each block prints and the mark… |
...
| `src/app/js/0800-providers.js` | **App.providers — CH's own document sections, as contract providers** | Control coverage and "Deviations from Security Guidelines" are CH concepts, not document-module ones. They used to be two branches of a kind switch i… |
...
| `src/doc/js/0590-ui-views-report-design/` (13 files) | **App.ui.views.reportDesign — RD-1..RD-7: the Report Design workspace** | The full-screen workspace that decides what the document contains, in what order, at what heading level, with what hand-authored sections, under whic… |
...
- **`App.docHost`** — `set`, `get`, `validate`, `sections`, `log`
- **`App.docProviders`** — `renderSection`
- **`App.docBlocks`** — `applySectionOrder`, `tableWording`, `headingsFor`, `withWording`
- **`App.docGen`** — `reportBlocks`, `sectionColumns`, `sectionContent`, `filterCounts`, `findTags`, `applyTags`, `docFilename`, `TAG_RE`, `emitDocument`, `sectionsOf`, `providerOf`
- **`App.generate`** — `buildImplementation`, `buildVerification`, `buildReport`, `buildControlReport`, `_gather`, `buildProcedure`, `procedureSteps`, `reportBlocks`, `sectionColumns`, `CONTROL_COLUMNS`, `buildControlSection`, `hostSections`, `controlLinkTerms`, `controlAnchor`, `relevanceCounts`, `relevanceKeys`, `relevanceLabel`, `REL_UNSET`, `relevanceKeyOf`, `relevanceFilter`, `hostBlocks`, `hostColumns`, `hostContent`, `filterCounts`, `emitDocument`, `findTags`, `applyTags`, `docFilename`, `TAG_RE`, `metaFields`, `deviceMeta`, `guidelineChildren`, `hasGuidelineDeviations`, `sectionContent`
- **`App.providers`** — `control`, `guidelines`
...
### Self-test blocks (21 blocks, 198 suites)
...
**PHASE-7 SELF-TEST SUITES (report · generate · determinism)** — `src/app/js/0510-phase-7-self-test-suites-report-generate-det/` (8 files)
...
- _control and guidelines as host providers (HOST-1)_
  - `018-providers.js:8` HOST-1 control coverage and guideline deviations are host providers
- _the subject and the row filter are host-declared (SUBJ-1 · FILT-1)_
  - `019-subject-filter.js:7` SUBJ-1 a host with no subject still gets a document
  - `019-subject-filter.js:71` FILT-1 the filter axis is declared by the host
- _the designer owns the description of a run (SESS-1)_
  - `020-session.js:7` SESS-1 the designer owns the description of a run
- _the application installs its host (HOSTBOOT-1)_
  - `020-session.js:70` HOSTBOOT-1 CH installs a document host at boot
...
- _report scope, section order & the options workspace (REL-7 · RPT-2/3/4)_
  - `170-rel-7.js:17` REL-7 
  - `170-rel-7.js:53` RPT-2 the report carries only the relevance categories you ask for
  - `170-rel-7.js:102` RPT-3 the report sections can be re-ordered, and it sticks
  - `170-rel-7.js:171` RPT-4 / RD-1 the Report Design workspace
...
**(unlabelled suites at line 34422)** — `src/doc/js/0900-doc-suites/020-blocks.js`

- _the block helpers (BLK-1)_
  - `line 11` BLK-1 ordering and wording are the module’s, not the host’s

**(unlabelled suites at line 34485)** — `src/doc/js/0900-doc-suites/030-host-object.js`

- _the host contract (HOSTOBJ-1)_
  - `line 10` HOSTOBJ-1 the module is driven by an explicit host object

**(unlabelled suites at line 34574)** — `src/doc/js/0900-doc-suites/040-conformance-mock-host.js`

- _a host that is not CH (CONF-1)_
  - `line 17` CONF-1 a minimal foreign host generates a document

**(unlabelled suites at line 34728)** — `src/doc/js/0900-doc-suites/050-markdown.js`

- _LaTeX-safe markdown (MD-1) · code spans (CODE-1)_
  - `line 11` MD-1 nothing hostile to LaTeX leaves the writer unescaped
  - `line 73` CODE-1 a code span is shaded, and a long one still wraps
- _line breaks and the rich-text box (BR-1 · RTX-2 · D-061)_
  - `line 114` BR-1 a line break renders wherever it is written (D-037)
  - `line 152` RTX-2 a table cell takes the same formatting a paragraph does
  - `line 195` D-061 a line break in a table cell reaches the preview

**(unlabelled suites at line 34984)** — `src/doc/js/0900-doc-suites/060-outline-and-references.js`

- _heading levels, numbering and hand-authored parts (DOC-1..DOC-4)_
  - `line 18` DOC-1/DOC-2 heading levels resolve and number themselves
  - `line 73` DOC-3 a cross-reference survives renaming and reordering
  - `line 127` DOC-4 a hand-authored section renders its parts in order
- _references and centring (REF-1 · CTR-1 · D-044)_
  - `line 177` REF-1 a cross-reference reaches the document as a link
  - `line 268` CTR-1 centring a section centres its heading too
  - `line 349` D-044 centring one section does not centre the document

**(unlabelled suites at line 35383)** — `src/doc/js/0900-doc-suites/070-widths-and-type.js`

- _automatic widths (AUTO-1 · AUTO-2 · D-036)_
  - `line 22` AUTO-1 a table that will not fit is laid out, not left to collapse
  - `line 92` AUTO-2 columns are measured in ems, not in characters
  - `line 156` D-036 a hand-set width still holds what cannot be broken
- _type sizes and what the preview shows (FNT-4 · FNT-6 · PRV-3)_
  - `line 232` FNT-4 every kind of text has a size, a weight and a slope
  - `line 273` FNT-6 the table\
  - `line 349` PRV-3 the preview reads the document back the way the page will

**(unlabelled suites at line 35794)** — `src/doc/js/0900-doc-suites/080-captions.js`

- _automatic captions (CAP-1)_
  - `line 61` CAP-1 every table is captioned, and the numbers agree

**(unlabelled suites at line 35941)** — `src/app/js/0600-v2-2-self-test-suites-report-design-markdown/` (17 files)

- _formatting profiles (FMT-1..FMT-4)_
  - `010-la-te-x-safe-markdown-md-1.js:37` FMT formatting profiles reach pandoc as YAML + a LaTeX preamble
...
- _quality of life (FNT-1 · OPT-1 · pane stickiness · tab)_
  - `060-tw-1-typing-a-width-and-being-told-when-it-d.js:136` FNT-1 three font sizes: text, table body, table header
  - `060-tw-1-typing-a-width-and-being-told-when-it-d.js:257` OPT-1 columns and groups live on the section row
...
- _a table narrower than the page (TW-3)_
  - `070-long-runs-are-marked-so-they-can-break-brk-1.js:107` TW-3 widths that do not fill the page make a narrower table
- _an unnumbered title level (TTL-1)_
  - `070-long-runs-are-marked-so-they-can-break-brk-1.js:162` TTL-1 a title takes no number and gives none away
- _the profile reaches the preview (PRV-2 · CAP-2 · NAM-2)_
  - `070-long-runs-are-marked-so-they-can-break-brk-1.js:233` PRV-2 the preview shows the formatting, not just the words
...
- _section names and introductions (NAM-1 · SEC-1)_
  - `080-nam-2-ccol-2-a-generated-section-names-its-h.js:94` NAM-1/SEC-1 a section names itself, and introduces itself
...
- _the document the designer controls (CODE-1 · TOC-1/2 · TTL-2 · CAP-3)_
  - `100-the-body-font-fnt-2.js:77` TOC-1/TOC-2 the contents list is a section like any other
  - `100-the-body-font-fnt-2.js:161` TTL-2 the automatic title block is a choice, and it is off
  - `100-the-body-font-fnt-2.js:201` CAP-3 the caption sits under its table, and carries its number
...
- _what the report is made of, saved with it (OPT-2 · COL-3)_
  - `110-the-box-shows-what-it-holds-rtx-1-rtx-2.js:148` OPT-2 the report\
  - `110-the-box-shows-what-it-holds-rtx-1-rtx-2.js:248` COL-3 an optional column carries its own default
...
- _references that reach the PDF (REF-1 · REF-2)_
  - `130-references-that-reach-the-pdf-ref-1-ref-2.js:3` REF-2 every mention of a control links to its coverage row
...
**(unlabelled suites at line 40004)** — `src/app/js/0610-suites-paths-and-the-driver-contract-tf-2/` (3 files)
