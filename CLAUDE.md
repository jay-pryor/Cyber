...
_Generated by `tools/gen-code-map.py` from `ch-config-tool.html` (29,651 lines). Do not edit by hand — re-run the script._
...
| `8899–9279` | **App.doc — DOC-1..DOC-4: the document outline, numbering and renderer** | Turn an ordered list of report BLOCKS into a numbered, cross-referenced markdown document. Owns four things the generators must not each re-invent: h… |
| `9280–9422` | **App.report** | Platform-agnostic MARKDOWN section builders (spec §10.3, DOD-8). Turns an adapter's DECLARED report columns into a markdown table, and the device met… |
| `9424–10517` | **App.generate** | Orchestrate the three INDEPENDENT generators (spec §7, §10): gather a device's applicable+complete items per dataset, call adapter generators + platf… |
| `10519–10837` | **App.ui.model** | PURE view-model helpers shared by the UI views: latest-version resolution, applicability map, and search/sort/filter of table rows. Kept pure so the… |
| `10839–10875` | **App.ui.activity** | The append-only Activity/Errors drawer log (spec §11.6). Nothing in the app fails silently — parse/validation/generation results land here. |
| `10877–12179` | **App.ui.tables** | Render a data-driven data-table tab to an HTML string (spec §11.2). Columns come from adapter.columns plus computed "Applies to"/"Status". NO hardcod… |
| `12181–12355` | **App.ui.views.onboard** | The Onboard tab (spec §11.4): one data-driven file slot per active-platform dataset), name/model/firmware fields, a gated Onboard button, capture hel… |
| `12357–12734` | **App.ui.views.generate** | The Generate tab (spec §11.5): device selector + three INDEPENDENT commands (Implementation/Verification/Reporting), each enabled iff the device is r… |
| `12736–13727` | **App.ui.views.devices** | The Devices tab (spec §11.3, DOD-9). Lists device configs grouped by baseId (latest = active; older = superseded read-only history), and a read-only… |
| `13729–14430` | **App.ui.views.controls** | The Control Manager tab (spec §18.3 CTL-1/CTL-4): view/add/edit/remove controls (title, type, description, assigned device configs). Decision assignm… |
| `14432–14570` | **App.ui.views.formats — VF-5 "Value formats" manager** | Create and edit the project's NAMED, REUSABLE value formats: a left list of formats, a right editor for the selected one, and — for the `options` kin… |
| `14572–15291` | **App.ui.views.help** | In-app manual (spec §11, Phase 8; rewritten review-15). One section per area of the app, reached from a section strip so no single page is a wall of… |
| `15295–17061` | **App.ui.app** | The UI shell + controller (spec §11.1). Builds the top bar, the data-driven tab nav (one tab per active-platform dataset + Devices/ Onboard/Generate)… |
| `24056–24857` | **App.docStore — DS-1..DS-6: the Report Design mutators** | Every write the Report Design workspace makes to the project — heading levels, custom sections and their parts, formatting profiles, section template… |
| `24859–25116` | **App.docTemplates — TPL-1..TPL-4: templates in and out, on their own** | Export and import section templates, formatting profiles and whole report templates as standalone files, independently of the project. Owns the merge… |
| `25118–25474` | **App.ui.mdPreview — PRV-1: markdown -> HTML for the live preview** | Render the EXACT markdown the Generate button downloads into browsable HTML, plus the outline needed to navigate it. Not a general markdown implement… |
| `25476–27066` | **App.ui.views.reportDesign — RD-1..RD-7: the Report Design workspace** | The full-screen workspace that decides what the document contains, in what order, at what heading level, with what hand-authored sections, under whic… |
| `29593–29648` | **App.bootstrap — LAST. Registers platforms and mounts the UI.** | Wire everything together at startup (spec §14). During Phase 0 it only renders self-tests on #selftest and a placeholder otherwise. |
...
**PHASE-3 SELF-TEST SUITES (ui.model + ui.tables render)** — lines `17064–17172`

- `17102` ui.model
- `17137` ui.tables render

**PHASE-4 SELF-TEST SUITES (diff · validation · store onboarding)** — lines `17175–17317`

- `17196` diff.triage
- `17205` validation.validateOnboarding
- `17217` store.onboardDevice (fresh + inheritance)
- `17261` store re-onboard versioning (§8.7)
- `17299` store.applicableItems / undecidedCount

**PHASE-5 SELF-TEST SUITES (completeness · store decisions · editors · gating)** — lines `17320–17468`

- `17350` completeness
- `17376` store.setDecision / setItemFields
- `17418` ui.tables decision editors (data-driven)
- `17451` ui.views.generate gating

**PHASE-6 SELF-TEST SUITES (devices view)** — lines `17471–17593`

- `17497` ui.views.devices list
- `17518` ui.views.devices detail (DOD-9)
- `17551` review-5 #1 device panels collapsible + per-control view

**PHASE-7 SELF-TEST SUITES (report · generate · determinism)** — lines `17596–18055`

- `17623` report markdown builders (v2.2)
- `17654` generate.buildImplementation
- `17693` generate.buildVerification & buildReport
- `17810` report-gen: descriptions + page setup + LaTeX safety (v2.2)
- `17860` T11 (v1.3) generation customisation
- `17965` review-9 apply-to toggle + AEST header + how-to-run
- `18001` generate determinism (DOD-7)
- `18022` review-10 (onboarded descriptions + .ps1 warning + control width)

**REVIEW-3 SELF-TEST SUITES** — lines `18058–18173`

- `18075` review-3 #1 generate scripts as .txt
- `18090` review-3 #3 / review-4 #1 policyList (prefixed internal key, stripped for display)
- `18139` review-3 #5a/#5b control types + import
- `18160` review-3 #6 device-detail search

**PHASE-8 SELF-TEST SUITES (DOD-11 portability + DOD checklist)** — lines `18176–18286`

- `18228` DOD-11 portability (mock platform, zero core edits)
- `18267` DOD checklist (programmatic items)

**PHASE-9 SELF-TEST SUITES (v1.1: dark mode · set-from-files · controls)** — lines `18289–24051`

- _v1.1 core (T9.x) — dark mode · set-from-files · control CRUD_
  - `18295` T9.1 dark mode (UI-only, determinism-safe)
  - `18320` T9.2 adapter parseAssignment
  - `18341` T9.3 store.applyDeviceAssignment (exact-set + atomic)
  - `18384` T9.6 store control CRUD
- _v1.2 overrides & device groups (T10.x)_
  - `18411` T10.3 store override + group mutators
  - `18478` T10.4 effective completeness, generation & manifest
  - `18523` T10.6 device-config override UI (divergence + editing)
  - `18575` T10.5 device groups UI (sections + deviations modal)
  - `18632` T10.7/T10.8 report deviations + determinism (overrides/groups)
- _review rounds 2-4, 7-8 — UI refinements + control refs in tables/report_
  - `18687` review-2: AEST banner time + resizable/wrapping value columns
  - `18709` review-7 collapse-all + Apply Control Mode
  - `18764` review-8 name-only picker + Apply-to action + resizable panels
  - `18827` T9.8/T9.9 control refs in tables & report
  - `18861` review-4 #2/#3/#4 UI: incomplete default + control layout/description
- _review rounds 12-13 — satisfaction · delete mode · relevance · undo/redo_
  - `18944` review-12 #1 per-device control satisfaction state
  - `19019` review-12 #2 Delete Mode + Undo
  - `19101` review-12 #3 Security Relevance column
  - `19153` review-12 #4 rationale + relevance columns in the decisions import
  - `19217` review-13 #1 delete selection tint
  - `19227` review-13 #2/#3 apply runs undo as one action, plus redo
  - `19296` UNDO-1 universal undo
  - `19404` review-13 #4 Control Manager per-device checkbox columns
- _review rounds 14-17 — relevance options · column widths · manual · editors_
  - `19482` review-14 REPORT + IRRELEVANT relevance options
  - `19590` review-15 starting column widths
  - `19621` review-15 Help manual
  - `19677` review-16 #1 blanking a text value
  - `19740` review-17 #1 tab order
  - `19765` review-17 #2 clear button sits beside the value box
  - `19778` review-17 #3 the status badge flips the item
  - `19851` review-17 #4 imsSettings is optional on upload
- _v2.1 value formats (VF-1…VF-8)_
  - `19963` VF value formats — inference, editors, enforcement
- _bulk apply & full-cell hit targets (BULK-1/2)_
  - `20219` BULK-1 apply the selected control to everything SHOWN
  - `20317` BULK-2 the tick columns are full-cell hit targets
- _hold-for-review · column filters · justification (HELD-1 · FIL-1 · JUS-1/2)_
  - `20357` HELD-1 flag for review without losing the value
  - `20470` FIL-1 per-column value filters
  - `20652` JUS-1/JUS-2 control satisfaction + justification
- _layout stability on tick (STAB-1 · STAB-3)_
  - `20794` STAB-1 a tick never changes a row height
  - `20859` STAB-3 a tick in the Control Manager does not move the page
- _control tags & column-wide device assign (TAG-1/2/3)_
  - `20887` TAG-3 the device column heading assigns the whole shown column
  - `20952` TAG-1/TAG-2 custom tags on controls
- _the sticky tools rail (SP-4 · SP)_
  - `21080` SP-4 the rail is actually sticky (CSS invariants)
  - `21195` SP sticky tools rail
- _v2.0 — retirement of the Settings dataset_
  - `21284` v2.0 Settings retirement (legacy projects still load)
- _columns & device read-across (CMD-1 · COL-1)_
  - `21418` CMD-1 the Applies-to device names read across, not down
  - `21472` COL-1 every column is hidable except the key column
- _divergence from guidelines (DIV-1/DIV-2)_
  - `21587` DIV-1/DIV-2 diverges from guidelines + the narrative behind it
- _custom security actions (CUS-1/CUS-2)_
  - `21758` CUS-1/CUS-2 Custom Security Actions
- _security relevance renamed (REL-1)_
  - `22036` REL-1 Security Relevance 
- _general platform notes (NOTE-1)_
  - `22104` NOTE-1 General Platform Notes
- _procedures & the Procedure report (PRO-1 · PRO-2)_
  - `22203` PRO-1/PRO-2 Procedures and the Procedure report
- _Control Manager column filters (CMF-1)_
  - `22368` CMF-1 Control Manager column filters
- _firewall rules & USB host interfaces (FW-1 · USB-1)_
  - `22474` FW-1/USB-1 firewall rules and USB host interfaces
- _column defaults · control coverage · order stability (COL-2 · RPT-1 · PRO-3)_
  - `22639` COL-2/RPT-1/PRO-3 columns, control coverage and the order editor
- _cell editing & manual device assignment (EDIT-1 · DEV-1)_
  - `22787` EDIT-1 a cell opens the row for editing
  - `22837` DEV-1 assigning register items to a device by hand
- _one captured-value map for the whole fleet (D-015)_
  - `23142` D-015 an assigned item keeps its decision on the new device
- _one stubborn package must not end the run (D-016)_
  - `23228` D-016 a package that refuses to uninstall must not kill the run
- _uninstall falls back to disable · reverse switches (PARTIAL-1 · REV-1)_
  - `23279` PARTIAL-1 a refused uninstall falls back to disable, and says so
  - `23324` REV-1 the reverse switches turn the script into a restore
- _a verification script carries no apply machinery (VER-5)_
  - `23385` VER-5 the verification script contains nothing that could change a device
- _the "(none)" filters survive the DOM (FIL-3)_
  - `23472` FIL-3 a 
- _report scope, section order & the options workspace (REL-7 · RPT-2/3/4)_
  - `23595` REL-7 
  - `23631` RPT-2 the report carries only the relevance categories you ask for
  - `23684` RPT-3 the report sections can be re-ordered, and it sticks
  - `23758` RPT-4 / RD-1 the Report Design workspace
- _bulk relevance & bulk decision (BULK-4)_
  - `23841` BULK-4 applying a relevance or a decision to many rows

**v2.2 SELF-TEST SUITES (Report Design: markdown · outline · templates)** — lines `27069–29591`

- _LaTeX-safe markdown (MD-1)_
  - `27110` MD-1 nothing hostile to LaTeX leaves the writer unescaped
- _heading levels, numbering and cross-references (DOC-1..DOC-4)_
  - `27173` DOC-1/DOC-2 heading levels resolve and number themselves
  - `27226` DOC-3 a cross-reference survives renaming and reordering
  - `27271` DOC-4 a hand-authored section renders its parts in order
- _formatting profiles (FMT-1..FMT-4)_
  - `27315` FMT formatting profiles reach pandoc as YAML + a LaTeX preamble
- _the designer's project writes (DS-1..DS-6)_
  - `27375` DS the Report Design mutators write deterministic, canonical state
- _templates in and out (TPL-1..TPL-4)_
  - `27467` TPL importing templates adds; it never deletes
- _the document the designer produces (RD-1..RD-7 · PRV-1)_
  - `27569` RD a designed document generates as one .md
- _metadata rows, guideline deviations, centring, section preview_
  - `27649` META-1 the provenance block is a chooseable list
  - `27698` GUIDE-1 Deviations from Security Guidelines
  - `27748` RD-8 centring and the per-section preview
  - `27798` PRV-1 the preview renders the document that will be generated
  - `27845` RD-2 the section editor exposes every part control
- _table column widths (TW-1)_
  - `27912` TW-1 a dragged column width reaches the PDF
- _automatic widths and the form a table takes (AUTO-1)_
  - `28122` AUTO-1 a table that will not fit is laid out, not left to collapse
- _header + first-column styling (TBS-1)_
  - `28194` TBS-1 a styled header row and first column
- _generated-section columns and widths (CCOL-1 · TW-2 · RD-9)_
  - `28322` CCOL-1/TW-2 a generated section chooses its columns and their widths
  - `28430` TW-1 typing a width, and being told when it does not add up
  - `28519` RD-9 a hand-authored section previews like a generated one
- _the width model measures ems (AUTO-2)_
  - `28565` AUTO-2 columns are measured in ems, not in characters
- _quality of life (FNT-1 · OPT-1 · pane stickiness · tab)_
  - `28631` FNT-1 three font sizes: text, table body, table header
  - `28737` OPT-1 columns and groups live on the section row
  - `28781` the workspace stops moving under you
- _long runs are marked so they can break (BRK-1)_
  - `28811` BRK-1 a run with no space in it is marked so it can wrap
- _what the preview shows (PRV-3)_
  - `28886` PRV-3 the preview reads the document back the way the page will
- _a table narrower than the page (TW-3)_
  - `28945` TW-3 widths that do not fill the page make a narrower table
- _an unnumbered title level (TTL-1)_
  - `29000` TTL-1 a title takes no number and gives none away
- _the profile reaches the preview (PRV-2 · CAP-2 · NAM-2)_
  - `29068` PRV-2 the preview shows the formatting, not just the words
  - `29139` NAM-2/CCOL-2 a generated section names its heading; a control names its type
- _automatic captions (CAP-1)_
  - `29232` CAP-1 every table is captioned, and the numbers agree
- _section names and introductions (NAM-1 · SEC-1)_
  - `29311` NAM-1/SEC-1 a section names itself, and introduces itself
- _values a person reads (HUM-1)_
  - `29429` HUM-1 a captured value is printed as a reading, not as JSON
- _satisfied with exception (EXC-1)_
  - `29502` EXC-1 a control can be satisfied with an exception
