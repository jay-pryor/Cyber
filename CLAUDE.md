...
_Generated by `tools/gen-code-map.py` from `ch-config-tool.html` (39,315 lines). Do not edit by hand — re-run the script._
...
| `1994–2125` | **App.storage — TF.2/TF.3: the shared vocabulary of the storage layer** | Status constants, the normalised error shape, and path helpers that the drivers and the folder store both speak. Kept in its own tiny module so neith… |
| `2127–2215` | **App.storage.driverMemory — TF.2: the reason any of this is testable** | A Map pretending to be a folder. Implements the driver contract exactly, including fault injection, so every policy above the driver line — snapshot… |
| `2217–2456` | **App.storage.driverFsa — TF.3: real files in a real folder** | The File System Access API behind the driver contract. Deliberately the THINNEST module in the storage layer, because it is the only one that cannot… |
| `2458–2738` | **App.storage.folder — TF.4: the layout, the snapshot policy, the guard** | Everything the storage layer decides, sitting above a driver that only moves named text. Owns the on-disk layout, the snapshot-before-write policy, p… |
| `2740–2853` | **App.storage.writer — TF.5: the 60s idle / 180s capped debounce** | The save POLICY, kept out of the folder store so both stay trivially testable. Coalesces a burst of edits into one write, guarantees only one write i… |
| `2855–4335` | **App.md — MD-1: LaTeX-safe markdown primitives** | The single choke point through which every data-derived string passes on its way into a generated document, exactly as App.util.html.esc is for the H… |
| `4336–4480` | **App.test — embedded test harness** | Tiny in-file test runner (spec §15). assert/assertEqual(deep)/ assertDeepEqual + suite/test registrar. Runs only on #selftest or via a dev button; MU… |
| `4581–4809` | **App.registry** | Platform/adapter registration (spec §5.3). The generic machinery discovers datasets/columns/decision-schemas by iterating the active platform — it ne… |
| `4811–6209` | **App.projectIo** | (De)serialize, schema-validate (Appendix A), and migrate the project file (spec §6.1, §7, §17.A). Canonical (de)serialization is the basis of DOD-2 (… |
| `6211–7621` | **App.store** | In-memory project + mutations + change events (spec §7.1). Holds the single mutable copy of canonical state; exposes deep-readonly snapshots and an e… |
| `8003–8855` | **App.adapters.android** | The Android (ADB) dataset adapters (spec Appendix B) + the two-layer shell-escaping helpers + tactical path/flatten/rebuild utilities. ALL Android/fo… |
| `8857–9378` | **App.platforms.androidAdb** | Assemble the Android (ADB) PlatformProfile from its dataset adapters plus platform-level output conventions (spec Appendix B). This is the ONLY place… |
| `9525–9554` | **App.diff** | Triage a snapshot's keys against the register (spec §8.1): split into new (append as undecided) and existing (inherited). Keys present in the registe… |
| `9556–9609` | **App.validation** | Cross-cutting structural checks beyond projectIo's schema validation (spec §7, §12): onboarding input validation and a whole-project pass. |
| `9611–9722` | **App.completeness** | Item completeness & device readiness (spec §6.5, §8.3). Wraps the adapter's isComplete with the optional REQUIRE_CONTROL_REF policy fold, and compute… |
| `9724–10001` | **App.valueFormats — v2.1 (VF-1…VF-8)** | Declare and enforce the SHAPE a decision value must take, so a free-text box stops being the answer to every question. Four built-in kinds (bool / nu… |
| `10003–10138` | **App.overrides** | The single choke point (spec §19.2.3, OVR-3) that resolves the default → group → device decision chain for an (item, device) pair, classifies value d… |
| `10140–11451` | **App.docFormat — FMT-1..FMT-4: named formatting profiles** | Everything about how the finished document PRESENTS itself — heading styling per level, page size and margins, page numbers, table of contents — as a… |
| `11452–12148` | **App.doc — DOC-1..DOC-4: the document outline, numbering and renderer** | Turn an ordered list of report BLOCKS into a numbered, cross-referenced markdown document. Owns four things the generators must not each re-invent: h… |
| `12149–12375` | **App.report** | Platform-agnostic MARKDOWN section builders (spec §10.3, DOD-8). Turns an adapter's DECLARED report columns into a markdown table, and the device met… |
| `12377–13657` | **App.generate** | Orchestrate the three INDEPENDENT generators (spec §7, §10): gather a device's applicable+complete items per dataset, call adapter generators + platf… |
| `13659–14026` | **App.ui.model** | PURE view-model helpers shared by the UI views: latest-version resolution, applicability map, and search/sort/filter of table rows. Kept pure so the… |
| `14028–14064` | **App.ui.activity** | The append-only Activity/Errors drawer log (spec §11.6). Nothing in the app fails silently — parse/validation/generation results land here. |
| `14066–15388` | **App.ui.tables** | Render a data-driven data-table tab to an HTML string (spec §11.2). Columns come from adapter.columns plus computed "Applies to"/"Status". NO hardcod… |
| `15390–15564` | **App.ui.views.onboard** | The Onboard tab (spec §11.4): one data-driven file slot per active-platform dataset), name/model/firmware fields, a gated Onboard button, capture hel… |
| `15566–16033` | **App.ui.views.generate** | The Generate tab (spec §11.5): device selector + three INDEPENDENT commands (Implementation/Verification/Reporting), each enabled iff the device is r… |
| `16035–17026` | **App.ui.views.devices** | The Devices tab (spec §11.3, DOD-9). Lists device configs grouped by baseId (latest = active; older = superseded read-only history), and a read-only… |
| `17028–17729` | **App.ui.views.controls** | The Control Manager tab (spec §18.3 CTL-1/CTL-4): view/add/edit/remove controls (title, type, description, assigned device configs). Decision assignm… |
| `17731–17869` | **App.ui.views.formats — VF-5 "Value formats" manager** | Create and edit the project's NAMED, REUSABLE value formats: a left list of formats, a right editor for the selected one, and — for the `options` kin… |
| `17871–18724` | **App.ui.views.help** | In-app manual (spec §11, Phase 8; rewritten review-15). One section per area of the app, reached from a section strip so no single page is a wall of… |
| `18728–19211` | **App.ui.folder — TF.6/TF.8/TF.9/TF.11: the folder, as the app sees it** | The single seam between the storage layer and the rest of the UI. Owns the folder store and the writer, the connect/reconnect/recovery/divergence scr… |
| `19213–21028` | **App.ui.app** | The UI shell + controller (spec §11.1). Builds the top bar, the data-driven tab nav (one tab per active-platform dataset + Devices/ Onboard/Generate)… |
| `28156–29280` | **App.docStore — DS-1..DS-6: the Report Design mutators** | Every write the Report Design workspace makes to the project — heading levels, custom sections and their parts, formatting profiles, section template… |
| `29282–29539` | **App.docTemplates — TPL-1..TPL-4: templates in and out, on their own** | Export and import section templates, formatting profiles and whole report templates as standalone files, independently of the project. Owns the merge… |
| `29541–30143` | **App.ui.mdPreview — PRV-1: markdown -> HTML for the live preview** | Render the EXACT markdown the Generate button downloads into browsable HTML, plus the outline needed to navigate it. Not a general markdown implement… |
| `30145–30449` | **App.ui.richText — RTX-1: the box shows what it holds** | Turn the {{...}} token markup App.md.rich reads into HTML a contenteditable box can be edited in, and turn the edited box back into tokens. Bold read… |
| `30451–33166` | **App.ui.views.reportDesign — RD-1..RD-7: the Report Design workspace** | The full-screen workspace that decides what the document contains, in what order, at what heading level, with what hand-authored sections, under whic… |
| `39257–39312` | **App.bootstrap — LAST. Registers platforms and mounts the UI.** | Wire everything together at startup (spec §14). During Phase 0 it only renders self-tests on #selftest and a placeholder otherwise. |
...
### Self-test blocks (13 blocks, 186 suites)

Tests are roughly a third of the file. Skip them unless the task is about a test.

**PHASE-0 SELF-TEST SUITES (vendored primitives + util)** — lines `4483–4579`

- `4488` util.clock
- `4496` util.html
- `4511` util.hash (SHA-256)
- `4539` util.crc32
- `4549` util.zip (determinism)

**PHASE-1 SELF-TEST SUITES (stable, registry, projectIo, store)** — lines `7624–8001`

- `7693` util.stable.stableStringify
- `7711` projectIo schema validation
- `7780` projectIo round-trip & determinism (DOD-2/DOD-7)
- `7832` T10.1 schema v3 (groups, overrides, migrate, prune)
- `7891` T10.2 App.overrides resolver
- `7967` store basics

**PHASE-2 SELF-TEST SUITES (Android adapters)** — lines `9381–9523`

- `9387` android.packages parse
- `9404` android.tactical flatten/rebuild
- `9459` decisionSchema / isComplete / validateDecision
- `9471` two-layer shell escaping (injection-safety, Appendix B)
- `9503` platform registration (android-adb)

**PHASE-3 SELF-TEST SUITES (ui.model + ui.tables render)** — lines `21031–21139`

- `21069` ui.model
- `21104` ui.tables render

**PHASE-4 SELF-TEST SUITES (diff · validation · store onboarding)** — lines `21142–21284`

- `21163` diff.triage
- `21172` validation.validateOnboarding
- `21184` store.onboardDevice (fresh + inheritance)
- `21228` store re-onboard versioning (§8.7)
- `21266` store.applicableItems / undecidedCount

**PHASE-5 SELF-TEST SUITES (completeness · store decisions · editors · gating)** — lines `21287–21435`

- `21317` completeness
- `21343` store.setDecision / setItemFields
- `21385` ui.tables decision editors (data-driven)
- `21418` ui.views.generate gating

**PHASE-6 SELF-TEST SUITES (devices view)** — lines `21438–21560`

- `21464` ui.views.devices list
- `21485` ui.views.devices detail (DOD-9)
- `21518` review-5 #1 device panels collapsible + per-control view

**PHASE-7 SELF-TEST SUITES (report · generate · determinism)** — lines `21563–22051`

- `21590` report markdown builders (v2.2)
- `21621` generate.buildImplementation
- `21660` generate.buildVerification & buildReport
- `21777` report-gen: descriptions + page setup + LaTeX safety (v2.2)
- `21830` T11 (v1.3) generation customisation
- `21961` review-9 apply-to toggle + AEST header + how-to-run
- `21997` generate determinism (DOD-7)
- `22018` review-10 (onboarded descriptions + .ps1 warning + control width)

**REVIEW-3 SELF-TEST SUITES** — lines `22054–22169`

- `22071` review-3 #1 generate scripts as .txt
- `22086` review-3 #3 / review-4 #1 policyList (prefixed internal key, stripped for display)
- `22135` review-3 #5a/#5b control types + import
- `22156` review-3 #6 device-detail search

**PHASE-8 SELF-TEST SUITES (DOD-11 portability + DOD checklist)** — lines `22172–22282`

- `22224` DOD-11 portability (mock platform, zero core edits)
- `22263` DOD checklist (programmatic items)

**PHASE-9 SELF-TEST SUITES (v1.1: dark mode · set-from-files · controls)** — lines `22285–28151`

- _v1.1 core (T9.x) — dark mode · set-from-files · control CRUD_
  - `22291` T9.1 dark mode (UI-only, determinism-safe)
  - `22316` T9.2 adapter parseAssignment
  - `22337` T9.3 store.applyDeviceAssignment (exact-set + atomic)
  - `22380` T9.6 store control CRUD
- _v1.2 overrides & device groups (T10.x)_
  - `22407` T10.3 store override + group mutators
  - `22474` T10.4 effective completeness, generation & manifest
  - `22519` T10.6 device-config override UI (divergence + editing)
  - `22571` T10.5 device groups UI (sections + deviations modal)
  - `22628` T10.7/T10.8 report deviations + determinism (overrides/groups)
- _review rounds 2-4, 7-8 — UI refinements + control refs in tables/report_
  - `22678` review-2: AEST banner time + resizable/wrapping value columns
  - `22700` review-7 collapse-all + Apply Control Mode
  - `22755` review-8 name-only picker + Apply-to action + resizable panels
  - `22818` T9.8/T9.9 control refs in tables & report
  - `22854` review-4 #2/#3/#4 UI: incomplete default + control layout/description
- _review rounds 12-13 — satisfaction · delete mode · relevance · undo/redo_
  - `22937` review-12 #1 per-device control satisfaction state
  - `23012` review-12 #2 Delete Mode + Undo
  - `23094` review-12 #3 Security Relevance column
  - `23146` review-12 #4 rationale + relevance columns in the decisions import
  - `23210` review-13 #1 delete selection tint
  - `23220` review-13 #2/#3 apply runs undo as one action, plus redo
  - `23289` UNDO-1 universal undo
  - `23397` review-13 #4 Control Manager per-device checkbox columns
- _review rounds 14-17 — relevance options · column widths · manual · editors_
  - `23475` review-14 REPORT + IRRELEVANT relevance options
  - `23583` review-15 starting column widths
  - `23616` review-15 Help manual
  - `23672` review-16 #1 blanking a text value
  - `23735` review-17 #1 tab order
  - `23760` review-17 #2 clear button sits beside the value box
  - `23773` review-17 #3 the status badge flips the item
  - `23846` review-17 #4 imsSettings is optional on upload
- _v2.1 value formats (VF-1…VF-8)_
  - `23958` VF value formats — inference, editors, enforcement
- _bulk apply & full-cell hit targets (BULK-1/2)_
  - `24214` BULK-1 apply the selected control to everything SHOWN
  - `24312` BULK-2 the tick columns are full-cell hit targets
- _hold-for-review · column filters · justification (HELD-1 · FIL-1 · JUS-1/2)_
  - `24352` HELD-1 flag for review without losing the value
  - `24465` FIL-1 per-column value filters
  - `24647` JUS-1/JUS-2 control satisfaction + justification
- _layout stability on tick (STAB-1 · STAB-3)_
  - `24797` STAB-1 a tick never changes a row height
  - `24862` STAB-3 a tick in the Control Manager does not move the page
- _control tags & column-wide device assign (TAG-1/2/3)_
  - `24890` TAG-3 the device column heading assigns the whole shown column
  - `24955` TAG-1/TAG-2 custom tags on controls
- _the sticky tools rail (SP-4 · SP)_
  - `25083` SP-4 the rail is actually sticky (CSS invariants)
  - `25198` SP sticky tools rail
- _v2.0 — retirement of the Settings dataset_
  - `25287` v2.0 Settings retirement (legacy projects still load)
- _columns & device read-across (CMD-1 · COL-1)_
  - `25421` CMD-1 the Applies-to device names read across, not down
  - `25475` COL-1 every column is hidable except the key column
- _divergence from guidelines (DIV-1/DIV-2)_
  - `25590` DIV-1/DIV-2 diverges from guidelines + the narrative behind it
- _custom security actions (CUS-1/CUS-2)_
  - `25761` CUS-1/CUS-2 Custom Security Actions
- _security relevance renamed (REL-1)_
  - `26039` REL-1 Security Relevance 
- _general platform notes (NOTE-1)_
  - `26107` NOTE-1 General Platform Notes
- _procedures & the Procedure report (PRO-1 · PRO-2)_
  - `26206` PRO-1/PRO-2 Procedures and the Procedure report
- _Control Manager column filters (CMF-1)_
  - `26371` CMF-1 Control Manager column filters
- _firewall rules & USB host interfaces (FW-1 · USB-1)_
  - `26477` FW-1/USB-1 firewall rules and USB host interfaces
- _column defaults · control coverage · order stability (COL-2 · RPT-1 · PRO-3)_
  - `26642` COL-2/RPT-1/PRO-3 columns, control coverage and the order editor
- _cell editing & manual device assignment (EDIT-1 · DEV-1)_
  - `26793` EDIT-1 a cell opens the row for editing
  - `26843` DEV-1 assigning register items to a device by hand
- _one captured-value map for the whole fleet (D-015)_
  - `27148` D-015 an assigned item keeps its decision on the new device
- _one stubborn package must not end the run (D-016)_
  - `27234` D-016 a package that refuses to uninstall must not kill the run
- _uninstall falls back to disable · reverse switches (PARTIAL-1 · REV-1)_
  - `27285` PARTIAL-1 a refused uninstall falls back to disable, and says so
  - `27330` REV-1 the reverse switches turn the script into a restore
- _a verification script carries no apply machinery (VER-5)_
  - `27391` VER-5 the verification script contains nothing that could change a device
- _the "(none)" filters survive the DOM (FIL-3)_
  - `27478` FIL-3 a 
- _report scope, section order & the options workspace (REL-7 · RPT-2/3/4)_
  - `27603` REL-7 
  - `27639` RPT-2 the report carries only the relevance categories you ask for
  - `27688` RPT-3 the report sections can be re-ordered, and it sticks
  - `27757` RPT-4 / RD-1 the Report Design workspace
- _bulk relevance & bulk decision (BULK-4)_
  - `27853` BULK-4 applying a relevance or a decision to many rows
- _a sort you can turn off (SORT-1)_
  - `28063` SORT-1 a column heading cycles ascending, descending, off

**v2.2 SELF-TEST SUITES (Report Design: markdown · outline · templates)** — lines `33169–38330`

- _LaTeX-safe markdown (MD-1)_
  - `33210` MD-1 nothing hostile to LaTeX leaves the writer unescaped
- _heading levels, numbering and cross-references (DOC-1..DOC-4)_
  - `33273` DOC-1/DOC-2 heading levels resolve and number themselves
  - `33328` DOC-3 a cross-reference survives renaming and reordering
  - `33382` DOC-4 a hand-authored section renders its parts in order
- _formatting profiles (FMT-1..FMT-4)_
  - `33431` FMT formatting profiles reach pandoc as YAML + a LaTeX preamble
- _the designer's project writes (DS-1..DS-6)_
  - `33497` DS the Report Design mutators write deterministic, canonical state
- _templates in and out (TPL-1..TPL-4)_
  - `33589` TPL importing templates adds; it never deletes
- _the document the designer produces (RD-1..RD-7 · PRV-1)_
  - `33691` RD a designed document generates as one .md
- _metadata rows, guideline deviations, centring, section preview_
  - `33774` META-1 the provenance block is a chooseable list
  - `33823` GUIDE-1 Deviations from Security Guidelines
  - `33877` RD-8 centring and the per-section preview
  - `33927` PRV-1 the preview renders the document that will be generated
  - `33984` RD-2 the section editor exposes every part control
- _table column widths (TW-1)_
  - `34051` TW-1 a dragged column width reaches the PDF
- _automatic widths and the form a table takes (AUTO-1)_
  - `34261` AUTO-1 a table that will not fit is laid out, not left to collapse
- _header + first-column styling (TBS-1)_
  - `34333` TBS-1 a styled header row and first column
- _generated-section columns and widths (CCOL-1 · TW-2 · RD-9)_
  - `34560` CCOL-1/TW-2 a generated section chooses its columns and their widths
  - `34679` TW-1 typing a width, and being told when it does not add up
  - `34768` RD-9 a hand-authored section previews like a generated one
- _the width model measures ems (AUTO-2)_
  - `34814` AUTO-2 columns are measured in ems, not in characters
- _quality of life (FNT-1 · OPT-1 · pane stickiness · tab)_
  - `34880` FNT-1 three font sizes: text, table body, table header
  - `35001` OPT-1 columns and groups live on the section row
  - `35045` the workspace stops moving under you
- _long runs are marked so they can break (BRK-1)_
  - `35076` BRK-1 a run with no space in it is marked so it can wrap
- _what the preview shows (PRV-3)_
  - `35151` PRV-3 the preview reads the document back the way the page will
- _a table narrower than the page (TW-3)_
  - `35212` TW-3 widths that do not fill the page make a narrower table
- _an unnumbered title level (TTL-1)_
  - `35267` TTL-1 a title takes no number and gives none away
- _the profile reaches the preview (PRV-2 · CAP-2 · NAM-2)_
  - `35338` PRV-2 the preview shows the formatting, not just the words
  - `35410` NAM-2/CCOL-2 a generated section names its heading; a control names its type
- _automatic captions (CAP-1)_
  - `35503` CAP-1 every table is captioned, and the numbers agree
- _section names and introductions (NAM-1 · SEC-1)_
  - `35590` NAM-1/SEC-1 a section names itself, and introduces itself
- _a numbered introduction (SEC-2) · control description (CCOL-3)_
  - `35703` SEC-2 an introduction can take a number of its own
  - `35806` CCOL-3 the control coverage table can carry the description
- _values a person reads (HUM-1)_
  - `35849` HUM-1 a captured value is printed as a reading, not as JSON
- _satisfied with exception (EXC-1)_
  - `35922` EXC-1 a control can be satisfied with an exception
- _the body font (FNT-2)_
  - `36012` FNT-2 the document chooses a font, and the preview shows it
- _the document the designer controls (CODE-1 · TOC-1/2 · TTL-2 · CAP-3)_
  - `36086` CODE-1 a code span is shaded, and a long one still wraps
  - `36125` TOC-1/TOC-2 the contents list is a section like any other
  - `36209` TTL-2 the automatic title block is a choice, and it is off
  - `36249` CAP-3 the caption sits under its table, and carries its number
- _the box shows what it holds (RTX-1 · RTX-2)_
  - `36324` RTX-1 a text box renders what it holds
  - `36467` RTX-2 a table cell takes the same formatting a paragraph does
- _what the report is made of, saved with it (OPT-2 · COL-3)_
  - `36512` OPT-2 the report\
  - `36612` COL-3 an optional column carries its own default
- _a table without a caption (CAP-4)_
  - `36653` CAP-4 a table can be left uncaptioned
  - `36733` SEC-3 the report no longer describes its own composition
  - `36766` NAM-3 heading, then name, whoever wrote the section
  - `36793` FNT-3 every size in the document, in one table
  - `36896` TTL-3 a title is styled as a title, not as an H1
- _references that reach the PDF (REF-1 · REF-2)_
  - `36968` REF-1 a cross-reference reaches the document as a link
  - `37059` REF-2 every mention of a control links to its coverage row
- _type, centring, table wording and placement (FNT-4 · CTR-1 · TBL-1 · SEC-4)_
  - `37147` FNT-4 every kind of text has a size, a weight and a slope
  - `37188` CTR-1 centring a section centres its heading too
  - `37269` TBL-1 a generated table names itself, and its groups take no heading
  - `37411` SEC-4 a section can start a page, or stay out of the contents
- _header & footer, generation, the paged preview (HDR-1 · GEN-TAB · PRV-4)_
  - `37511` HDR-1 the header and footer are configured on their own
  - `37665` GEN-TAB the document is named, and its placeholders filled in
- _what the second round of use found (D-036..D-039)_
  - `37722` D-036 a hand-set width still holds what cannot be broken
  - `37796` D-044 centring one section does not centre the document
  - `37843` BR-1 a line break renders wherever it is written (D-037)
  - `37881` PRV-4 the preview can be laid out as pages
- _a break in a cell, and a first column with a size (D-061 · FNT-6)_
  - `37918` D-061 a line break in a table cell reaches the preview
  - `37976` FNT-6 the table\
- _whitespace as a measurement (SPC-1)_
  - `38053` SPC-1 empty space is asked for in millimetres

**(unlabelled suites at line 38332)** — lines `38332–39255`

- _paths and the driver contract (TF.2)_
  - `38406` TF.2 storage path helpers
  - `38425` TF.2 driverMemory honours the driver contract
- _real DOMExceptions, not synthetic stand-ins (D-067)_
  - `38469` D-067 a real DOMException is normalised, not waved through
- _the snapshot policy (TF.4 · requirements 7.1)_
  - `38546` TF.4 snapshots are due on a 5-minute floor and pruned to 40
- _the canonical write (TF.4 · requirements 6.2, 7.1)_
  - `38606` TF.4 the canonical write
- _the concurrent-write guard (TF.4 · requirements 10)_
  - `38661` TF.4 the guard refuses to clobber
- _quarantine and outputs (TF.4 · requirements 8, 9)_
  - `38736` TF.4 quarantine preserves the bytes a human could repair
  - `38768` TF.10 generated artifacts land unpacked under Outputs/
- _the debounced writer (TF.5 · requirements 6.2)_
  - `38802` TF.5 the writer coalesces, caps, and never spins
- _the app wired to a folder (TF.6 · TF.8 · TF.10)_
  - `38923` TF.6 the app boots into the right folder state
- _the writer over the real folder store (TF.4 + TF.5)_
  - `39197` TF.5 the writer driving the folder store end to end
